...
pub struct AStar {
    ...
    pub fscore: BTreeMap<(usize, usize), i64>,
...
impl AStar {
        ...
        while open_set.len() > 0 {
            ...
            for neighbor in neighbors {
                ...
                if cur_score.is_none() || tentative_gscore < *self.gscore.get(&neighbor).unwrap() {
                    ...
                    let fscore_temp = tentative_gscore + self.evaluate_fscore(neighbor);
    ...
    fn initialize_fscore(&self) -> BTreeMap<(usize, usize), i64> {
        let mut fscore: BTreeMap<(usize, usize), i64> = BTreeMap::new();
        let start_val = self.evaluate_fscore(self.start)
    ...
    fn evaluate_fscore(&self, node: (usize, usize)) -> i64 {
        return ((self.start.0 - node.0) + (self.start.1 - node.1)) as i64;
    }
