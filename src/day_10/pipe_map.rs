...
impl PipeMap {
    ...
    fn are_connected(&mut self, one: (usize, usize), two: (usize, usize)) -> bool {
        ...
        if con_pipe.is_some(){
            for con in con_pipe.unwrap() {
                let res = ((two.0 as i32 + con.0) as usize, (two.1 as i32 + con.1) as usize);
                if res == one {
                    return true;
                }
            }
    ...
    pub fn print_distances(&self){
        let mut grid : Vec<Vec<String>> = vec![vec![" ".to_string(); self.grid.len()]; self.grid.len()];
